...
- `.github/workflows/desktop-release.yml` — Windows desktop installers (.exe app image + .msi) on PR/push builds and tag releases
...
`desktop-release.yml` runs on a **windows-latest** runner on pushes and
PRs to `main`/`Alpha` AND on the same `v*` tags (plus manual dispatch). It:
...
- Zips the app image into a **portable** `Curio-Windows-{version}-portable.zip`
  and uploads both it and the `.msi` to the GitHub release on tags. On
  PR/push runs the same two files are uploaded as run artifacts
  (`curio-desktop-windows-*`, 7-day retention) so every PR push produces
  the proper `.exe` for download — the release-only steps (release body,
  prerelease detection, `action-gh-release` publish) are gated on
  `startsWith(github.ref, 'refs/tags/')`. `RELEASE_VERSION` is only set for
  tag runs; PR/push runs keep the module's default `1.0.0` package version
  (and the zip's default name) because jpackage rejects non-numeric versions.

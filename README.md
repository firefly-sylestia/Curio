...
> **Read this before switching Online mode on:** direct messages are end-to-end encrypted on supported builds. The message body is encrypted on the sending device before upload and decrypted only on a participant device. Curio stores ciphertext, a nonce, encryption metadata, and participant-specific encrypted conversation-key envelopes; private keys remain in each device's Android Keystore. Community cards, replies, reactions, reports, usernames and profile text are not covered by DM encryption and remain protected by Supabase access controls and moderation rules.
>

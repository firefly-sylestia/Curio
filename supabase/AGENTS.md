...
- **24-hour cards.** `community_cards.expires_at` defaults to `now() + interval '24 hours'` and the insert policy refuses anything longer than 25 hours. Every read filters `expires_at > now()`, so expiry holds even before the sweep runs; `curio_purge_expired_cards()` is housekeeping (optionally scheduled with `pg_cron`).

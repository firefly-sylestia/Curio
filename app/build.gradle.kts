...
// task before packaging. The task runs inside Gradle rather than launching a
// compiled Kotlin main class; this avoids variant output/classpath differences
// across Android Gradle Plugin versions. It is intentionally generated in CI
// rather than committed because it is a derived binary.
tasks.register("buildTopicDatabase") {
    ...
    doLast {
        ...
        if (outputDb.exists() && !outputDb.delete()) {
            throw GradleException("Could not replace generated database: ${outputDb.absolutePath}")
        }

        Class.forName("org.sqlite.JDBC")
        val connection = java.sql.DriverManager.getConnection("jdbc:sqlite:${outputDb.absolutePath}")
        connection.use { db ->
            ...
            val insert = db.prepareStatement("INSERT INTO topics VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)")
            insert.use { statement ->
                var totalCount = 0
                val parser = groovy.json.JsonSlurper()
                jsonDir.listFiles { file -> file.extension == "json" }
                    ?.sortedBy { it.name }
                    ?.forEach { jsonFile ->
                        if (jsonFile.name == "topic_index.json") return@forEach
                        val topics = parser.parse(jsonFile) as? List<*>
                            ?: throw GradleException("${jsonFile.name}: root must be a JSON array")
                        topics.forEach { rawTopic ->
                            @Suppress("UNCHECKED_CAST")
                            val topic = rawTopic as? Map<String, Any?>
                                ?: throw GradleException("${jsonFile.name}: topic must be an object")
                            fun requiredText(name: String): String = topic[name] as? String
                                ?: throw GradleException("${jsonFile.name}: topic missing string field '$name'")
                            val action = topic["exploreAction"] as? Map<*, *>
                                ?: throw GradleException("${jsonFile.name}: topic missing exploreAction")
                            statement.setString(1, requiredText("id"))
                            statement.setString(2, requiredText("categoryId"))
                            statement.setString(3, requiredText("subtype"))
                            statement.setString(4, requiredText("name"))
                            statement.setString(5, requiredText("teaser"))
                            statement.setString(6, topic["imageUrl"] as? String ?: "")
                            statement.setString(7, topic["byline"] as? String ?: "")
                            statement.setString(8, (topic["tags"] as? List<*>)?.let { groovy.json.JsonOutput.toJson(it) } ?: "")
                            statement.setInt(9, (topic["tier"] as? Number)?.toInt() ?: 1)
                            statement.setString(10, action["verb"] as? String ?: "")
                            statement.setString(11, action["targetName"] as? String ?: "")
                            statement.setInt(12, (action["durationMinutes"] as? Number)?.toInt() ?: 0)
                            statement.setString(13, action["instruction"] as? String ?: "")
                            val pageCount = topic["pageCount"] as? Number
                            if (pageCount == null) statement.setNull(14, java.sql.Types.INTEGER)
                            else statement.setInt(14, pageCount.toInt())
                            val episodeCount = topic["episodeCount"] as? Number
                            if (episodeCount == null) statement.setNull(15, java.sql.Types.INTEGER)
                            else statement.setInt(15, episodeCount.toInt())
                            statement.setString(16, topic["altPageLabel"] as? String ?: "")
                            val altPageCount = topic["altPageCount"] as? Number
                            if (altPageCount == null) statement.setNull(17, java.sql.Types.INTEGER)
                            else statement.setInt(17, altPageCount.toInt())
                            statement.addBatch()
                            totalCount++
                        }
                    }
                statement.executeBatch()
                logger.lifecycle("✓ topics.db: $totalCount topics")
            }
            db.createStatement().use { statement ->
                statement.executeUpdate("CREATE INDEX index_topics_categoryId ON topics(categoryId)")
                statement.executeUpdate("CREATE INDEX index_topics_name ON topics(name)")
            }
        }
    }
}

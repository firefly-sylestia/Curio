...
abstract class CurioDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        /**
         * v457 — WHERE A HIGHLIGHT'S WORDS ARE ([ReaderMarkEntity]'s offsets).
         *
         * -1 for every row that already exists, which is the honest backfill:
         * a mark made before these columns carried only its WORDS, so the
         * renderer must keep searching for them (see [ReaderMarkEntity]) — and
         * -1 is exactly the value that says "unknown", never 0 (which is a real
         * offset: the first character of a block).
         */
        val MIGRATION_23_24 = object : Migration(23, 24) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL(
                    "ALTER TABLE `reader_marks` ADD COLUMN `startIndex` " +
                        "INTEGER NOT NULL DEFAULT -1"
                )
                db.execSQL(
                    "ALTER TABLE `reader_marks` ADD COLUMN `endIndex` " +
                        "INTEGER NOT NULL DEFAULT -1"
                )
            }
        }

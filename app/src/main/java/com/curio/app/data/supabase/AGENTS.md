...
- **UI-safe errors.** Failures surface through `onlineAuthMessage` (auth) and `communityMessage` (REST): rate limits, unconfirmed emails and invalid credentials stay actionable; anything unrecognised collapses to one generic line so a raw response body can never reach the UI. A stale session and a server that predates the app are CONDITIONS with an action attached ("That session has expired…", "Curio needs its server update. Paste supabase/schema.sql…") and must be mapped BEFORE the pass-through of already-written text, because `SupabaseClient` reports them as plain `IllegalStateException`s. The stale-server condition covers a missing FUNCTION, a missing TABLE (PostgREST `PGRST205`) and a missing embedded RELATIONSHIP (`PGRST200`) — the last two are what a not-yet-pasted table looks like from the client, and neither may surface as PostgREST's own sentence about a schema cache. `CommunityError.transport` marks an IOException family failure, where the OUTCOME is unknown rather than refused — a write uses it to reconcile instead of claiming failure.
- **A refused token is refreshed and the request RETRIED, not reported.** `SupabaseClient.executeBody` sees the 401, calls `sessionRefresher` (`OnlineAccount.refreshForRetry` — blocking, on the REST layer's own IO thread; it reuses a token another call already minted, answers `Unreachable` for a device with no network and `Refused` when the refresh token itself was rejected) and re-sends the SAME request once with the new token. Requests that carried no token are never retried (a 401 on sign-in is about the credentials). REST timeouts are generous on purpose (connect 15s, read/write 30s): OkHttp's 10s defaults fired while a send was still on the wire, which read as "the message never sent" for a message that had arrived. Keep `refreshSessionBlocking` out of `executeBody`'s retry path — a refresh that goes through the retrying path recurses.

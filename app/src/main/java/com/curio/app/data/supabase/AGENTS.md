...
- **DM cryptographic contract.** `CurioDmCrypto` uses 32-byte AES-256-GCM keys, fresh 12-byte nonces, 128-bit tags, and the canonical UTF-8 `dmConversationId` as AAD. Every ciphertext names one positive key version; receivers batch-fetch and install that exact device envelope before decrypting. RSA-OAEP-SHA256 envelopes are only for the registered target device, and private RSA keys never leave Android Keystore. Logs may include only redacted IDs, lengths, versions, state, and exception classes — never tokens, key material, plaintext, or ciphertext.

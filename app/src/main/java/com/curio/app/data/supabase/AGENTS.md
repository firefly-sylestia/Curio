...
- **UI-safe errors.** Failures surface through `onlineAuthMessage` (auth) and `communityMessage` (REST): rate limits, unconfirmed emails and invalid credentials stay actionable; anything unrecognised collapses to one generic line so a raw response body can never reach the UI. A stale session and a missing server function are CONDITIONS with an action attached ("That session has expired…", "Curio needs its server update. Paste supabase/schema.sql…") and must be mapped BEFORE the pass-through of already-written text, because `SupabaseClient` reports both as plain `IllegalStateException`s. `CommunityError.transport` marks an IOException family failure, where the OUTCOME is unknown rather than refused — a write uses it to reconcile instead of claiming failure.

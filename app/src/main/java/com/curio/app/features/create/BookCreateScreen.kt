...
private data class DraftChapter(val title: String, val content: String)

@Composable
fun BookCreateScreen(onClose: () -> Unit, onCreated: () -> Unit = onClose) {
    ...
    val chapters = remember { mutableStateListOf(DraftChapter("Chapter 1", "")) }
    val palettes = listOf(
    ...
    fun saveBook() {
        ...
        scope.launch {
            val chapterModels = chapters.mapIndexed { index, chapter ->
                BookChapter(index + 1, chapter.title.ifBlank { "Chapter ${index + 1}" }, 0, 0, chapter.content.trim().replace(Regex("\\s+"), " ").take(180))
            }
            val chapterText = chapters.mapIndexed { index, chapter ->
                "Chapter ${index + 1}: ${chapter.title.ifBlank { "Untitled" }}\n${chapter.content.trim()}"
            }.joinToString("\n\n")
            val topic = CurioTopic(
                id = newCreatedEntryId(), categoryId = CategoryId.BOOKS, subtype = "Book", name = title.trim(),
                teaser = description.trim().ifBlank { "A book created in Curio." }, imageUrl = "",
                exploreAction = ExploreAction("Read", title.trim(), 0, "Read this personal book at your own pace."),
                byline = author.trim(), synopsis = description.trim().ifBlank { null }, chapters = chapterModels
            )
            val entry = CurioEntry(
                id = newCreatedEntryId(), topic = topic, format = CaptureFormat.Marginalia,
                ...
                ), title = title.trim(), capturedAtMillis = System.currentTimeMillis()
            )
    ...
    Box(Modifier.fillMaxSize().background(MaterialTheme.colorScheme.background)) {
        Column(Modifier.fillMaxSize()) {
            Row(Modifier.fillMaxWidth().statusBarsPadding().padding(horizontal = 16.dp, vertical = 10.dp), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.SpaceBetween) {
                Surface(onClick = onClose, shape = CircleShape, color = MaterialTheme.colorScheme.surfaceContainer, border = BorderStroke(1.dp, MaterialTheme.colorScheme.outlineVariant.copy(alpha = .45f)), modifier = Modifier.size(44.dp)) {
                    Box(contentAlignment = Alignment.Center, modifier = Modifier.fillMaxSize()) { CurioIcon(CurioIcons.Close, "Close book creator", tint = MaterialTheme.colorScheme.onSurface, size = 20.dp) }
                }
                Column(horizontalAlignment = Alignment.CenterHorizontally) {
                    ...
                    Text(if (writtenChapters == 0) "A book begins with a page" else "$writtenChapters chapter${if (writtenChapters == 1) "" else "s"} written", style = MaterialTheme.typography.labelSmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                }
                Spacer(Modifier.size(44.dp))
            }
            Column(Modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(horizontal = 14.dp, vertical = 5.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
                Surface(shape = RoundedCornerShape(30.dp), color = MaterialTheme.colorScheme.surfaceContainerLow, border = BorderStroke(1.dp, MaterialTheme.colorScheme.outlineVariant.copy(alpha = .42f)), modifier = Modifier.fillMaxWidth()) {
                ...
                Surface(shape = RoundedCornerShape(28.dp), color = MaterialTheme.colorScheme.surface, border = BorderStroke(1.dp, MaterialTheme.colorScheme.outlineVariant.copy(alpha = .42f)), modifier = Modifier.fillMaxWidth()) {
                ...
                Surface(shape = RoundedCornerShape(28.dp), color = MaterialTheme.colorScheme.surfaceContainerLow, border = BorderStroke(1.dp, MaterialTheme.colorScheme.outlineVariant.copy(alpha = .42f)), modifier = Modifier.fillMaxWidth().animateContentSize(tween(220, easing = FastOutSlowInEasing))) {
                    Column(Modifier.padding(20.dp), verticalArrangement = Arrangement.spacedBy(13.dp)) {
                        ...
                        chapters.forEachIndexed { index, chapter ->
                            Surface(onClick = { editingChapter = index; showChapterEditor = true }, shape = RoundedCornerShape(19.dp), color = MaterialTheme.colorScheme.surface, border = BorderStroke(1.dp, MaterialTheme.colorScheme.outlineVariant.copy(alpha = .42f)), modifier = Modifier.fillMaxWidth()) {
                ...
                Surface(onClick = ::saveBook, enabled = canCreate, shape = RoundedCornerShape(20.dp), color = if (canCreate) MaterialTheme.colorScheme.primary else MaterialTheme.colorScheme.surfaceContainer, modifier = Modifier.height(46.dp)) {
                    Row(Modifier.padding(horizontal = 16.dp), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(7.dp)) {
                        AnimatedContent(targetState = saving, transitionSpec = { fadeIn(tween(120)) + scaleIn(initialScale = .92f) togetherWith fadeOut(tween(90)) + scaleOut(targetScale = .92f) }, label = "book-save-icon") { busy ->
                            CurioIcon(if (busy) CurioIcons.Edit else CurioIcons.Check, null, tint = MaterialTheme.colorScheme.onPrimary, size = 18.dp)
                        }
        ...
        AnimatedVisibility(visible = showChapterEditor, enter = fadeIn(tween(160)) + scaleIn(initialScale = .98f), exit = fadeOut(tween(120)) + scaleOut(targetScale = .98f), modifier = Modifier.fillMaxSize()) {
            ChapterEditorOverlay(
                ...
                onDelete = { if (editingChapter in chapters.indices && chapters.size > 1) chapters.removeAt(editingChapter); showChapterEditor = false },
                onSave = { chapterTitle, content ->
                    val draft = DraftChapter(chapterTitle, content)
                    if (editingChapter in chapters.indices) chapters[editingChapter] = draft else chapters.add(draft)
...
@Composable
private fun ChapterEditorOverlay(chapterNumber: Int, initialTitle: String, initialContent: String, canDelete: Boolean, onDismiss: () -> Unit, onDelete: () -> Unit, onSave: (String, String) -> Unit) {

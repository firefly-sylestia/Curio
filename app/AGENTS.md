...
- **Curio's social language — `features/community/SocialComponents.kt` is the ONLY place social chrome is defined.** Box cards (`SocialCard`), pill actions (`SocialPill` / `SocialIconPill`, tones ACCENT / NEUTRAL / DESTRUCTIVE), person cards (`SocialPersonCard`), thread cards (`SocialThreadCard`), empty cards (`SocialEmptyCard`), day rules, the frosted social search pill, and the reaction palette (`SocialReactions`). Every social surface (wall, Friends, a profile, a conversation) composes from this kit, so a name, a portrait and a set of actions can never drift between screens. A new social row/card that hand-rolls its own `Surface` + `TextButton` is a bug.

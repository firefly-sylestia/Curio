...
- **Two views, one behaviour.** `grid` is a saveable flag toggled from the header; the list leads with the row's ORDER in a numbered disc (this page's promise is a sequence), the grid leads with the order plus a status DOT beside it and a whisper of that status inside the tile's own fill, because a chip's words are illegible at tile size. Both open the same `IncursionDetailSheet` — a `ModalBottomSheet` (the app's ONE detail language; it was an `AlertDialog`), carrying the order disc, the title with its meta line, an Essential chip, the synopsis, the `WATCH FIRST` prerequisite and the six-way status picker.

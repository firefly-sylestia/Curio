...
- **Fifteen hidden writing gestures, each its own switch (`PersonalGestures.kt`, v413).** `JournalGestureTool` is the catalogue — FIFTEEN enum entries, each carrying the bit it owns, its label, its two-line explanation (the gesture first, then what it does) and the `JournalGesture` it runs — and `AppPreferences.journalGestureToolsState` is ONE int holding them all as bits (`KEY_JOURNAL_GESTURES`), so a new tool needs no new key, no new load line and no new read site. The recogniser is ONE `Modifier.journalGestures(enabled) { … }` with a single `awaitEachGesture` loop: it counts the fingers that land, follows their centre, and decides tap vs swipe from the centre's whole journey (a tap must not have wandered) and where it ENDED (a swipe's direction). It is attached to `PersonalCanvas`'s modifier (and therefore sits BEFORE the canvas's own `clickable`, per the modifier-order rule), reads in `PointerEventPass.Initial`, and CONSUMES only once two fingers are down — multi-finger input is the one thing a text field has no use for, so with the section off the modifier returns itself untouched and typing, scrolling, the caret and selection behave exactly as before. **Single-finger gestures are NOT in the recogniser**: a finger on the writing belongs to the caret and the scroll, so the two single-finger tools ride the page's BLANK space, where `PersonalWritingPage`'s `.clickable { editor.focusLastLine() }` becomes a `combinedClickable` whose double-tap and long-press are null unless their tool is on (with both null it is the clickable it replaces — `combinedClickable` only arms a double-tap timeout when it is given one). Every action is `PersonalEditorState.runJournalGesture`, written on the editor's OWN public API (`restoreRemovedRow`, `setAlign`, `moveBlock`, `cycleListStyle`, `selectPage`/`clearPageSelection`, `insertTitleLine`, `insertPlainText`, `applyMarker`, `applyFont`, `removeRow`), so a gesture can never do what a button cannot and the page's auto-save sees ordinary writing. **Four actions are handed back to the PAGE, because they are not the editor's to do:** `onSaveNow` (the page owns the row and the debounce), `onUndo` (see below), and `onCopy`/`onPaste` (only a composable can reach `LocalClipboardManager` — the editor must stay a plain state holder). **The page's UNDO is a ring of the last `UndoDepth` 20 documents, coalesced to one snapshot per `UndoIntervalMs` 900ms**, pushed from `PersonalWritingPage`'s ONE `editor.onDocChanged` callback (so typing, the dock, a photo, a voice note and the gestures are all covered) and popped back through `editor.replace` — which deliberately does NOT announce a change, so the page sets its own `doc` too or the next save would write the undone words back; the clock is then reset so the undo itself is never re-pushed. Two reaches are worth knowing when reading the recogniser: a FOUR-finger tap is the last count with a tool of its own and a higher count is swallowed (a stray finger must never fall back to a lower count's action), and the double-tap slots are tracked PER FINGER COUNT so a two-finger tap can never be the first half of a three-finger double-tap. **`removeRow`, not `removeBlock`, is what the delete gesture calls** — it is the one that hands the removed line to the floating Undo pill, so a four-finger tap is always recoverable. One reach is deliberate and documented on the row: **alignment is the PAGE's** in this app (all four dock buttons call the same `setAlign`), and a MARKER dresses a list row, so cycling marks gives the line a bullet with it.

...
- **Single Support & diagnostics page (v24):** Support & diagnostics (`features/support/SupportScreen.kt`, route `SUPPORT`) is the ONE page for updates, feedback, replay intro, and the project link — the old Settings → About page (`SettingsPage.ABOUT`, `SETTINGS_ABOUT` route, `AboutSection`, `CurioUpdateCheckRow`) was removed. The page is reachable from Profile's "Support & diagnostics" row, Settings → Safety & support → "Support & diagnostics", and the Home drawer. **In-app updates (v24):** the update check queries Google Play Core (`com.google.android.play:app-update`, `CurioInAppUpdate` in `infrastructure/`, root host in `MainActivity`) for a FLEXIBLE in-app update first and falls back to the GitHub release check (full expandable release notes) on sideloads / when Play has nothing. **Installer gate (v25):** the Play path is gated on `CurioInAppUpdate.isInstalledFromPlay` (`getInstallerPackageName == com.android.vending`) — Play Core's availability answer alone is NOT a reliable Play-vs-sideload test (sideloads can still report UPDATE_AVAILABLE, which used to surface a bogus "Update available on Google Play" card on GitHub installs). Non-Play installs skip the Play query entirely. **Kotlin gotcha (v25 CI fix):** `CurioInAppUpdateHost()` is a TOP-LEVEL composable, so the object's members must be qualified — `CurioInAppUpdate.isInstalledFromPlay(...)` / `CurioInAppUpdate.finishInstall(...)`. Unqualified references from the host were CI compile errors (`Unresolved reference`).

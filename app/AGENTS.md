...
- **An OPEN conversation is LIVE.** `DirectMessageScreen` polls `SocialApi.messagesSince(token, other, me, newestConfirmed)` every `LIVE_TICK_MS` (**1.2s**) and merges what arrived — a strict `created_at > newest` window, so a tick is a few hundred bytes, and the anchor ignores optimistic `local-` bubbles because those carry the phone's clock. An arrival clears the typing row and stamps a read receipt. The "is typing…" read runs on a 1.5s tick beside it. The inbox (`FriendsScreen`) refreshes threads + requests on a slower `INBOX_TICK_MS` (**5s**) WITHOUT touching the loading state, so an unread badge appears where the user is looking. Never go back to "load once on entry" — that is the bug where a received message only appeared after leaving the screen.

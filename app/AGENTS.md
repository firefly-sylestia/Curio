...
### The card ladder — cards are WHITE on the cream page (v408)
- **`background` is the PAGE and stays cream (`CurioColors.SoftCream`).** Everything that reads as a CARD, TILE, SHEET, TAB or DIALOG is white in light mode. The container ladder in `CurioTheme.kt` (`CurioLightColorScheme`) climbs ABOVE the page instead of below it: `surfaceContainerLowest` / `surfaceContainerLow` = white (cards), `surfaceContainer` = a nested block inside a card, `surfaceContainerHigh` = the floating pill/dialog step, `surfaceContainerHighest` = the anchor step that still reads as page family. Dark keeps its pitch-black page with the ladder starting at `#121212` and striding `#161616 → #1C1C1C → #242424 → #2C2C2C`.
- **Why:** the light containers used to be DARKER cream than the page (Low `#F0E8D6` on a `#F7F0E4` page), so a card sat one warmth-step below the page — and on a lane wash (Home/Profile/Cabinet/Settings wear the Spin lane's tint of that same cream) the two collapsed into one flat field. Member report: "the background and the cards and tabs some were blending too much … the journals in the home screen the preview of them get the background color and blends".
- **Never tint a card back toward the page to "soften" it.** `settingsCardTintLift()` (light branch) is WHITE with 8% of the hero ink, not the page background — it was the page itself, which is what made `CurioSettingsCard`'s 30% lerp resolve to paler cream on cream. `curioDialogContainerColor()` (light) is the white panel with the rose whisper, not `surfaceContainerHigh`.
- **Translucent white is not a card fill.** The old family (`Color.White.copy(alpha = 0.68f)` over the page, `.copy(alpha = 0.62f)` for rail chips, `0.70f` for the search field) resolved to very nearly the page on a wash; every one of them is now an OPAQUE scheme step. If you need a card, take a ladder token; if you need glass, use the liquid-glass components.
- **Separations are formulaic:** `outline` / `outlineVariant` carry real alpha again (light 0.24 / 0.13, dark 0.26 / 0.14), dividers halve them, and an opaque card that needs an edge takes `border = BorderStroke(1.dp, MaterialTheme.colorScheme.outlineVariant)` on its `Surface` — or, for a `Modifier` chain, the `border` goes AFTER the `background` (a border earlier in the chain is painted over by the fill).
...
- **`SettingsRowEntry.plain = true`** (Online mode, Recycle bin, Updates, Support & diagnostics) renders a full-width row with NO icon tile, taller padding, and copy free to wrap to three lines (`SettingsOptionRow(plain = …)` → `SettingsOptionCopy(subtitleMaxLines = 3)`). Its divider goes flush (`SettingsOptionDivider(startInset = 0.dp)`) instead of the 53dp icon inset. Those four are exactly the rows whose one-line subtitles the old designed cards truncated.

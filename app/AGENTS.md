...
- **v27v — custom pet designs ALWAYS win + custom-pet procedural defaults.**
  ...
  layers, so they stay enabled automatically. Applied by `saveAsNewPet()`
  and when saving updates a custom slot (plain Curie saves keep the
  working design exactly as edited).

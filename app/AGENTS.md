...
- **The wall's look (v388, user decision):** post cards are COMPACT by default (tighter padding, 32dp portraits — `AppPreferences.communityRoomyWall` is the roomy escape hatch in User experiments) and wear the app's rose ACCENT family, not the capture paper creams; the floating Post pill hides while the wall scrolls DOWN and returns when the finger goes up or the page reaches the top (the same `snapshotFlow` scroll-slope pattern as Home's `+`). A QUOTE renders through `SocialPullQuote` (SocialComponents.kt) on BOTH the composer preview and the wall — one composable, one shape: an accent rule carrying the block, an oversized opening mark, the serif face, and the credit tied on by a short dash — so what the writer previewed is what the wall shows.
- **Follows + quote-reposts (v389, user picks after the ladder shipped):** `member_follows` (follower/followed, PK pair, block-outranks-follow insert policy) is the Follow button's whole server side behind `CommunityApi.follow/unfollow/followingIds` — the wall's **Following chip** filters the feed to `authorId in followingIds` client-side (ids read once per open, off = Everyone), and a member profile's second hero pill flips optimistically and rolls back on a refused call. A **quote-repost** is a fourth post KIND (`REPOST`): the composer opens with the quoted card attached (the wall's quote icon), the writer's words go to `quote_words` and the quoted card's id to `quote_source_id`, and `CARD_COLUMNS` joins `quote_source:community_cards!community_cards_quote_source_id_fkey(...)` so the read arrives complete — `SocialTextPost` renders the words ABOVE a nested quoted card (a quoted QUOTE keeps its pull-quote), and the composer's preview nests the same shape (`TextPostPreview(quoted=)`), so the preview does not lie. A REPOST's own row still carries reactions and replies as its own post.

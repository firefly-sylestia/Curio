...
### Voice notes in a page, and the page's own manners (v389)
...
- **A book the catalog does not have gets REAL chapters (`PersonalChapter` + `personal_books.chaptersJson`, migration 16→17).** Open Library keeps a table of contents per EDITION, so `BookEnrichment.openLibraryChapters` looks the work up by title (the edition's own normalized title must match, or the contents belong to another book), walks its editions for the first table with `>= 3` rows (fewer is a "Contents" line), and stores the names plus the page range each spans when the edition gives one. `rememberBookChapters` is the ONE resolver both the book page and the chapter page read: the catalog's list when the book came from the lane, else the learned one, else nothing (numbered rows). Room is at **v17** with `MIGRATION_16_17` (that column plus `personal_notes.kind` / `topicId` / `topicName` / `categoryId`, all non-null defaults), so an existing library reads exactly as it did before.
...
- **Two views, one behaviour.** `grid` is a saveable flag toggled from the header; the list leads with the row's ORDER in a numbered disc (this page's promise is a sequence), the grid leads with a status-coloured bar across the tile's top edge because a chip's words are illegible at tile size. Both open the same `IncursionDetailSheet` (synopsis, `WATCH FIRST` prerequisite, and the six-way status picker).
...
- **Social avatars:** Community identity uses only `profiles.avatar_style` (0–27) and the code-drawn `features/community/SocialAvatar.kt` renderer, drawn CHIBI (v388 correction — the v386 detail pass read "weird eyes and not-matching eyebrows and a weird neck tint"): one big dark oval eye with a deep centre and two white highlights plus a single lid stroke, brows mirrored about the midline from the SAME points so they always match, and a neck in the skin's own shade (not an ink wash) with a thinner jaw shadow. Users choose from 28 local vector/canvas styles in Online Mode; no avatar bitmap, URI, upload API, or media column belongs in the online layer. The first 16 are deliberately DISTINCT characters (beanie, bob, glasses, curls, headphones, wizard hat, leaf crown, goggles, beret, pigtails, top bun, freckles, hood, helmet) and the v3xx52 soft set adds twelve feminine silhouettes (waves, high ponytail, twin braids, hime cut, bob with a bow, half-up bun, flower crown, star clips, pixie with a heart clip, waves with glasses, afro puff, braided crown) across a wider spread of skin and hair tones — all drawn on a shared 100×100 design grid so one avatar looks the same at 24dp in a chip and at 96dp on a profile. The v386 detail pass underneath it still holds: a real BUST (shoulder silhouette, collar trim, fold lines, a lit shoulder), a HEAD with an oval skull tapering into a jaw and chin plus ears, material ACCESSORIES (a ribbed beanie cuff and a fuzzy pompom, a bent wizard cone with a stitched band, headphone cups with a mic boom, lensed goggles, a helmet with a glare sweep and a riveted collar, interlocking braids, rosette flowers, folded bows, shadowed clips). The drawing is CLIPPED to the disc (`clipPath`) so a bust that reaches the frame's bottom can never show square corners outside the circle, and every shade is DERIVED from that row's own garment/skin/hair tones — the 28-row palette table and the 0–27 count are untouched.
...
- **Zoom is two fingers only.** `Modifier.pinchToZoom` in `BookReaderScreen.kt` waits for a second pointer before it claims anything, because on a reader the single-finger drag is the scroll or the page turn. A reflowed book zooms by TYPE size (`ReaderLook.textScale`); a PDF zooms by `graphicsLayer` scale/pan with the pager stood down (`userScrollEnabled`).

...
- **The "+" sheet now has four doors:** a journal page, a book, a note on a topic (opens a name-input sheet, the topic id is the lowercased name) and a to-do list (a page with `kind = PAGE_KIND_TODO`). The journal editor accepts optional `initialTopicId` / `initialTopicName` / `initialCategoryId` / `initialKind` params so the route carries the topic context into the saved entity.

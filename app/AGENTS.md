...
- **~~THE PAGE'S COLOUR REACHES ITS OWN CHROME.~~ WITHDRAWN — SEE v438 BELOW.** v437 briefly pushed the page's own colour into `journalPaperRaised()`, the dock's tools (`journalDoorAccent(journalAccent)`) and `JournalTopBar` (handed the argb, because the bar stands outside the paint provider). **None of that is true any more**: the member withdrew it in one line ("the journal tools etc dont get the color they stay like before only the preview gets the color"). Read the v438 section before touching any of those three.
...
### v438 — the page always keeps a line to type in, and the journal's colour goes back to its doors

- **THE JOURNAL'S COLOUR IS THE DOORS' AGAIN, AND THE PAGE IS NEVER PAINTED (`journalPaper`, `journalPaperRaised`, `journalInk`, `JournalAccentSheet`).** The member withdrew the whole of v429's "Paint the page too" option: *"ykw remove the paint the page so the journal tools etc dont get the color they stay like before only the preview gets the color"*, having seen the fault it carried — a paper tinted toward an arbitrary pick left the page's ink and its neighbouring surfaces at contrasts nothing had measured, so words blended into the fill. So: `journalPaper()` is `lerp(warm, personalAccent(), …)` with no branch, `journalPaperRaised()` is back to `personalAccentInk()`, `journalInk()` is the theme's `onSurface` flat, the dock lights with `personalAccentInk()`, `JournalTopBar` is the theme's accent again, and the sheet's switch is gone. **Where the colour DOES live: the doors** — `journalDoorAccent(accentArgb)` on the journal list's spine (`JournalListScreen`) and Home's journal chips (`PersonalHome`) — which is what the member means by "the preview".
- **THE PAINT MECHANISM IS DELETED, NOT JUST DISABLED — AND THAT IS THE POINT.** `JournalPagePaint`, `LocalJournalPagePaint` and all three `CompositionLocalProvider` sites are gone, along with `journalOn()` and the painted-page branch that made `journalInk()` answer dark-on-light by luminance. **Nothing asks a page's colour for a TINT any more.** A switch nobody can flip whose plumbing still reaches five surfaces is exactly the arrangement that produces the next bug.
- **`PersonalNoteEntity.pagePainted` AND ITS COLUMN STAY (no schema change).** The field still round-trips at its default and `PersonalPageMeta.pagePainted` still carries it, so no Room migration is needed — the same treatment `profiles.avatar_style` got when the portrait picker went. `PersonalDao.setPagePainted` is now unreferenced; deleting the column would need the member's word and a migration.

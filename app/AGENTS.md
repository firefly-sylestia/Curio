...
### v446 — the reader's floating pills reach Edit profile, and a download that shows its bar

- **THE IDENTITY EDITOR IS BUILT OF THE READER'S OWN CHROME OBJECTS (`pillTitleStyle`, the head row, the foot's two pills).** The member: *"in edit profile make the edit profile be that floating header pill style lke pdf raeder, and same floating cancel and save pill"*. The page's head had been a large title under a bare back glyph; it is now **the reader's own head**: a 50dp way-back circle and a 50%-radius capsule carrying "Edit profile", both filled `colorScheme.surface` and lifted `shadowElevation = 10.dp` (the same lift `ReaderTopPill` wears), and the foot's Cancel / Save changes are **two floating pills of the same 50dp height and roundness** rather than a full-width band. The spec's "one short supporting sentence" moved into the body as a quiet line above the picture instead of being deleted with the title, and the type hierarchy is unchanged (nothing every-heading-bold) — this is a re-skin of the page's frame, not a re-design of the spec. **No SQL, no migration, nothing new to store.**
- **BADGES SAY A LINE'S STATE, IN THE STATE'S OWN COLOUR (`EditBadge`, `EditSectionHeading(badge = …)`, `EditFlatValue`).** The member: *"giving some items proer badge colors etc"*. One object carries every badge — the tint at 12% as the fill, the tint itself for the words, `RoundedCornerShape(50)` — so: the ACCOUNT heading says `Curio` in the accent when signed in and `Signed out` in the neutral tone when it is not; the locked EMAIL value trades its bare padlock glyph for a **`Locked` badge**; and the username's one state line (the rule being broken, the server's verdict, "sign in to claim") is now a badge tinted **error** when it is a refusal and **accent** otherwise. A badge for a LABEL is uppercased; a badge carrying a sentence passes `uppercase = false`, because a sentence has no business being shouted.

- **THE DICTIONARY'S DOWNLOAD WEARS A REAL BAR (`downloading` / `downloadProgress` in the sheet's own row).** Follow-up polish on v445's offline door: the row already said a percentage, but a percentage that ticks every few hundred kilobytes reads as a stuck number on a phone connection, so the row now carries a 4dp bar under it (the track at 10% of the reader's ink, the fill in the reader's accent) sized from the same 0..1 fraction the download already reported. It is drawn from two boxes rather than a progress-indicator API, so the reader's paper and accent decide its look on every Material version this module builds against.
